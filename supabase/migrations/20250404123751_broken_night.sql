/*
  # Fix Permission Denied for Users Table
  
  1. Problem:
    - Getting "permission denied for table users" error
    - The current RLS policies aren't allowing proper access to the users table
    - Public access to user data is needed for frontend components
  
  2. Changes:
    - Create more permissive RLS policies for the users table
    - Ensure authenticated and public roles can access their own data
    - Make admin access more robust with direct and fallback methods
    - Cleanup duplicate or conflicting policies
  
  3. Security:
    - Maintain basic security principles
    - Only allow users to see their own data unless they're an admin
*/

/*
  # Fix Authentication Session Persistence and Admin Roles
  
  1. Problem:
    - Session tokens are not persisting correctly
    - Admin role checking needs improvement
    - Authentication status tracking is missing
  
  2. Solution:
    - Add authenticated_status column to users table
    - Fix function parameter name conflicts
    - Improve session persistence through better database policies
*/
...
CREATE OR REPLACE FUNCTION public.set_user_as_admin(email_input TEXT)
RETURNS BOOLEAN AS $$
DECLARE
  target_id UUID;
BEGIN
